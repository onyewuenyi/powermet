...
| Power convergence | `convergence.py`, `schema.CONVERGENCE_METRICS`, `metrics.add_convergence_metrics()` | CdynTot / LkgPwr derivation (`cdyn_pf()`), `converge()` (gap, cut needed, trend, builds-to-target, verdict), `plan()` (techniques ranked by the component they move, mW → pF via V²f), renderers | `budgets`, `techniques` | `test_convergence.py` |
