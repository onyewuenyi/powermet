...
| Techniques registry | `techniques.py` | `Technique` (problem, mechanism, trade-off, considerations, data needed, `reduces` component, `assess()`), `TechniqueResult.saving_for(component)`, `assess_all()`, catalogue and result rendering | `selection`, `schema` | `test_methodologies.py::test_techniques_catalog_and_assessment` |
...
| Ingest pipeline | `pipeline.py` | `extract_run()` (extract → normalise → lineage → pivot → intent), `verify_run_consistency()`, `join_metric()` / `pivot_fub_records()` / `attach_identity()` / `stamp_provenance()` / `pivot_perf_records()`, `store_results()` (single writer), `ingest_runs()` serial, `plan_jobs()` + `RunResult.save_partition()/load_partition()` + `merge_partitions()` for scheduler fan-out | most of the above | `test_pipeline_units.py` (each building block, append regressions, consistency), `test_extract_pipeline.py` |
...
| Power budgets | `budgets.py` | `Budget` / `load_budgets()` (TOML with per-milestone tolerance, any convergence metric), `check_budgets()` with trend, FUB coverage and status, `classify_status()`, `fmt_metric()` | `selection`, `metrics`, `schema` | `test_closure.py`, `test_convergence.py` |
| Power convergence | `convergence.py`, `schema.CONVERGENCE_METRICS`, `metrics.add_convergence_metrics()` | Cdyn / leakage power derivation (`cdyn_pf()`), `converge()` (gap, cut needed, trend, builds-to-target, verdict), `plan()` (techniques ranked by the component they move, mW → pF via V²f), renderers | `budgets`, `techniques` | `test_convergence.py` |
...
| **Comparative analysis** | `anomalies.py` | `RULES` registry (`Rule`: compares, bug class, severity, technique, needs, fn); `anomalies()` -> `AnomalyReport` with `Finding`s (owner, evidence, threshold, new / persisting) and the cleared list; `render_rules()`; catalog `record_anomalies()` | `selection`, `config`, `schema` | `test_analysis.py` (planted bugs found, regression clears, rules skip without inputs) |
| Time-based profile | `timeprofile.py` | `summarize_profile()` per workload x op: average, peak window, peak/avg, max step, energy, pJ/op, reconciliation with the averaged report | `selection` | `test_analysis.py` |
| Power groups / profile adapters | `extract/power_groups.py`, `extract/power_profile.py` | cell-group split per hierarchy (columns read by name); design power per time window with `t_start_ns`/`t_end_ns` kept as `power_profile.parquet` via `pipeline.pivot_profile_records()` | `extract/base.py` | `test_analysis.py` (literal reports, units, optional flag) |
| Ownership | `identity.py` (`FubSpec.owner`), `pipeline.attach_identity()` | optional `owner` column in the FUB map carried on every dataset row | – | `test_analysis.py` |
